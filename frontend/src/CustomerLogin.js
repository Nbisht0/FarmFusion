...
function CustomerLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (res.data.success && res.data.user) {
        ...
        navigate("/customer-dashboard", { state: { user: res.data.user } });
      } else {
  ...
  return (
    <div style={{minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#dcfce7"}}>
      <form onSubmit={handleLogin} style={{background: "white", padding: "2rem", borderRadius: "1rem", boxShadow: "0 4px 6px rgba(0,0,0,0.1)", width: "100%", maxWidth: "400px"}}>
        <h2 style={{textAlign: "center", color: "#047857", marginBottom: "2rem", fontSize: "1.5rem"}}>Customer Login</h2>
        ...
        <button type="button" onClick={() => navigate("/customer-register")} style={{width: "100%", background: "#22c55e", color: "white", padding: "0.75rem", borderRadius: "0.5rem", fontWeight: "bold", border: "none", cursor: "pointer", marginTop: "1rem"}}>
          Register
        </button>

        <button type="button" onClick={() => navigate("/")} style={{width: "100%", background: "#6b7280", color: "white", padding: "0.75rem", borderRadius: "0.5rem", fontWeight: "bold", border: "none", cursor: "pointer", marginTop: "0.5rem"}}>
          Back Home
        </button>
      </form>
    </div>
